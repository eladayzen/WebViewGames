...
// people who share this device. It is not a global board: that would need a
// shared Firestore collection and write rules that do not exist, and nothing
// here should be designed as though it were coming.
//
...
// point here answers "no board" rather than throwing, and the game is fully
// playable in a browser with the section hidden. That is not a nicety: it is
// how the game is developed and tested.
//
 ...
 *
 * ONE ROW PER RUN, not per profile (playtest round 13). The first version kept
 * each profile's best and dropped the rest, so that one player could not fill
 * the board. Amit's call is the opposite and it is the right one for what this
 * became: with a top-5 plus a window around your own run, a player's several
 * runs are the texture that makes the board worth reading -- "I am two places
 * off my own best" is a thing only visible if the runs are all there.
 *
 * Every row carries its TRUE RANK in the full list, assigned here and never
 * recomputed downstream. A window showing rows 34-38 has to say 34-38; a
 * section renumbering itself 1..5 would quietly claim the player is in the top
 * five.
 *
 ...
 * ambiguous only between runs that are identical anyway, and therefore
 * interchangeable.
 *
 * If that run is already inside the leaders, there is NO second section: it is
 * highlighted where it sits. Repeating one row twice on a short screen reads as
 * a fault, and landing in the top five already answers "how did I do".
 *
...
export function resultSections(rows, justScored, topN = 5, above = 2, below = 2) {
  ...
  let idx = rows.findIndex((r) => r.isYou && Math.round(r.score || 0) === target);
  if (idx < 0) return { top, window: [] };
  // Already shown among the leaders.
  if (idx < topN) return { top, window: [] };

  const from = Math.max(topN, idx - above);
  // `below` rows after it WHERE THEY EXIST -- Amit was explicit that what is
  // behind you matters more than what is ahead, because it is the part that
  // says the run was worth something.
